...
def parseYAML(source):
    ...
    yamlLineRegex = re.compile(
        "^(?P<indent> *)((?P<itemMarker>- +)(?P<itemContent>.*)|((?P<key>[\w\.]+)(?P<keyExtra>: *))?(?P<content>.*))\s*$"
    )
    ...
    def parseYAML_inner(source, indent):
        ...
        for linenumber, line in source:
            ...
            if line.strip() == "" or commentLineRegex.match(
                line
            ...
            if not parsedLine:
                raise Exception(
                    "Unparseable YAML line %d: %s" % (linenumber, line.rstrip())
                )
            ...
            lineContent = (
                parsedLine.group("content") or parsedLine.group("itemContent") or ""
            ).strip()
            ...
            if lineItemMarker:
                ...
                if returnItem is None:
                    ...
                    raise Exception(
                        "Bad YAML, got a list item while working on a %s on line %d: %s"
                        % (returnItem.__class__.__name__, linenumber, line.rstrip())
                    )
            ...
            # - dict item
                ...
                if returnItem is None:
                    ...
                    raise Exception(
                        "Bad YAML, got a dict value while working on a %s on line %d: %s"
                        % (returnItem.__class__.__name__, linenumber, line.rstrip())
                    )
            ...
            # - data - one or more lines of text
                ...
                if returnItem is None:
                    ...
                    raise Exception(
                        "Bad YAML, got a value while working on a %s on line %d: %s"
                        % (returnItem.__class__.__name__, linenumber, line.rstrip())
                    )
                if returnItem:
                    returnItem = yamlValue(
                        returnItem + "\n" + lineContent, returnItem.linenumber
